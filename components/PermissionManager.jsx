...
const modules = {
  ...
  Jobs: {
    name: "Jobs",
    permissions: ["View Jobs", "Aiign Job", "Send Email", "View Details"],
  },
  Contracts: {
    name: "Contracts",
  ...
  Quotes: {
    name: "Quotes",
    permissions: ["Add Quotes", "View Quotes"],
  },
  Scheduler: {
    name: "Scheduler",
    permissions: ["View Scheduler"],
  },
  StockReport: {
    name: "Stock Report",
    permissions: ["View Stock Report"],
  },
  Clients: {
    name: "Clients",
    permissions: ["Add Clients", "View Clients"],
  },
  Invoices: {
    name: "Invoices",
...
const PermissionsManager = ({
  ...
  const getConditionalPermissions = (moduleName, permission, isAdding) => {
    ...
    if (moduleName === "Receives" && isAdding) {
      ...
      if (
        ["Receives Party", "Receives Investor", "Receives Product"].includes(
          permission
        )
      ) {
        const currentModulePerms = modulePerms[moduleName] || [];
    ...
    if (moduleName === "Payments" && isAdding) {
      ...
      if (
        ["Payment Party", "Payment Investor", "Payment Product"].includes(
          permission
        )
      ) {
        const currentModulePerms = modulePerms[moduleName] || [];
  ...
  const getPermissionsToRemove = (moduleName, permission, isRemoving) => {
    ...
    if (moduleName === "Receives" && isRemoving) {
      ...
      if (permission === "Add Receives") {
        ["Receives Party", "Receives Investor", "Receives Product"].forEach(
          (perm) => {
    ...
    if (moduleName === "Payments" && isRemoving) {
      ...
      if (permission === "Add Payments") {
        ["Payment Party", "Payment Investor", "Payment Product"].forEach(
          (perm) => {
            if (currentModulePerms.includes(perm)) {
              permissionsToRemove.push(perm);
            }
  ...
  useEffect(() => {
    if (currentPermissions && !isInitialized) {
      ...
      if (
        ...
        currentPermissions.permissions.modules
      ) {
        ...
        Object.keys(currentPermissions).forEach((moduleName) => {
          if (
            Array.isArray(currentPermissions[moduleName]) &&
            currentPermissions[moduleName].length > 0
          ) {
            processedPermissions[moduleName] = [
      ...
      if (Array.isArray(modulesData) && modulesData.length > 0) {
        modulesData.forEach((module) => {
          if (
            ...
            console.log(
              `Processed module: ${module.parent}`,
              module.permissions
            );
          }
        });
      ...
      Object.keys(processedPermissions).forEach((moduleName) => {
        if (
          processedPermissions[moduleName] &&
          processedPermissions[moduleName].length > 0
        ) {
      ...
      if (
        ...
        existingPermissions.permissions.modules
      ) {
    ...
    if (isInitialized && currentPermissions) {
      console.log(
        "Re-initializing due to currentPermissions change:",
        currentPermissions
      );
  ...
  const sendPermissionsToParent = useCallback(
    (perms) => {
      if (!isInitialized) {
        return;
      }
      ...
      const myModules = Object.entries(perms)
        .filter(
          ([parent, permissions]) => permissions && permissions.length > 0
        )
  ...
  const handleModulePermission = useCallback(
    (moduleName, permission) => {
      console.log(
        `Toggling permission: ${permission} for module: ${moduleName}`
      );

      // Check if permission is disabled
      if (isPermissionDisabled(moduleName, permission)) {
        console.log(
          `Permission ${permission} is disabled for module ${moduleName}`
        );
        return;
      }
      ...
      setModulePerms((prev) => {
        ...
        if (permissionIndex === -1) {
          ...
          const additionalPermissions = getConditionalPermissions(
            moduleName,
            permission,
            true
          );
          additionalPermissions.forEach((additionalPerm) => {
            if (!newPermissions[moduleKey].includes(additionalPerm)) {
              ...
              console.log(
                `Auto-added permission ${additionalPerm} to ${moduleName}`
              );
            }
          });
        } else {
          ...
          const permissionsToRemove = getPermissionsToRemove(
            moduleName,
            permission,
            true
          );
          permissionsToRemove.forEach((permToRemove) => {
            ...
            console.log(
              `Auto-removed permission ${permToRemove} from ${moduleName}`
            );
          });
          ...
          if (newPermissions[moduleKey].length === 0) {
            ...
            console.log(
              `Removed module ${moduleName} as it has no permissions`
            );
          }
        }
  ...
  const selectAllModulePermissions = useCallback((moduleName) => {
    ...
    setModulePerms((prev) => {
      ...
      console.log(
        `Selected all permissions for ${moduleName}:`,
  ...
  const clearAllModulePermissions = useCallback((moduleName) => {
    ...
    setModulePerms((prev) => {
      ...
      console.log(
        `Cleared all permissions for ${moduleName}. New state:`,
        newPermissions
      );
      return newPermissions;
    });
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {moduleData.permissions.map((permission) => {
                    const isSelected = isModulePermissionSelected(
                      moduleName,
                      permission
                    );
                    ...
                    return (
                      <button
                        ...
                        title={
                          isDisabled &&
                          moduleName === "Receives" &&
                          modulePerms[moduleName]?.includes("View Receives")
                            ? "This permission is disabled when 'View Receives' is selected"
                            : isDisabled &&
                              moduleName === "Payments" &&
                              modulePerms[moduleName]?.includes("View Payments")
